...
const Promo = ({ onLang, lang }) => {
    ...
    useEffect(() => {
        console.log(btnRefs.current[0].value);
        for(let i = 0; i<btnRefs.current.length; i++){
            if(localStorage.getItem('Language') === btnRefs.current[i].value){
                btnRefs.current.forEach(item=>item.classList.remove('promo__change_lang-active'));
                btnRefs.current[i].classList.add('promo__change_lang-active');
            }
        }
        // btnRefs.current.forEach(item => {
        //     switch (localStorage.getItem('Language')) {

        //         case btnRefs.current[0].value:
        //             item.classList.remove('promo__change_lang-active');
        //             btnRefs.current[0].classList.add('promo__change_lang-active');
        //             break;

        //         case btnRefs.current[1].value:
        //             item.classList.remove('promo__change_lang-active');
        //             btnRefs.current[1].classList.add('promo__change_lang-active');
        //             break;
                    
        //         case btnRefs.current[2].value:
        //             item.classList.remove('promo__change_lang-active');
        //             btnRefs.current[2].classList.add('promo__change_lang-active');
        //             break;
